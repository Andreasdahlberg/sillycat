...
static void BatteryCheckAndNotify(void);
...
{
    static event_type event = {0}; //TODO: Why static?
...
static void BatteryCheckAndNotify(void)
{
            ...
            libPower_GetBatteryVoltage() < LOW_VOLTAGE_MV)
    {
        ...
        INFO("Low battery level");

        event = NewEvent(EVENT_LOW_BATTERY);
        NotifyListeners(&event);
    }
}
*/
